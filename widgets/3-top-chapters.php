...
class Custom_Widget_3_Top_Chapters extends Widget_Base {

	public function get_name() {
		return '3-top-chapters';
	}

	public function get_title() {
		return esc_html__( '3-Top Chapters', 'custom-hello-theme' );
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			'hero_section',
			[
				'label' => esc_html__( 'Hero Section', 'custom-hello-theme' ),
				'tab' => Controls_Manager::TAB_CONTENT,
			]
		);

		$this->add_control( 'scroll_text_mb', [ 'type' => Controls_Manager::TEXT, 'label' => 'Scroll Text (MB)', 'default' => 'Swipe down' ] );
		$this->add_control( 'scroll_text_dk', [ 'type' => Controls_Manager::TEXT, 'label' => 'Scroll Text (DK)', 'default' => 'Scroll down to discover' ] );
        ...
        $this->start_controls_section(
			'who_section',
			[
				'label' => esc_html__( 'Who We Are', 'custom-hello-theme' ),
				'tab' => Controls_Manager::TAB_CONTENT,
			]
		);

        $this->add_control( 'who_title', [ 'type' => Controls_Manager::TEXTAREA, 'label' => 'Title', 'default' => 'Montfort is a global commodity trading and asset investment company.' ] );
        $this->add_control( 'who_content', [ 'type' => Controls_Manager::WYSIWYG, 'label' => 'Content', 'default' => '<p>We trade, refine, store, and transport energy and commodities. We also invest in related assets and provide innovative services with integrity and efficiency to create long-term value for our clients.</p>' ] );
        $this->add_control( 'who_link_label', [ 'type' => Controls_Manager::TEXT, 'label' => 'Link Label', 'default' => 'Who we are' ] );
        $this->add_control( 'who_link_url', [ 'type' => Controls_Manager::URL, 'label' => 'Link URL', 'default' => [ 'url' => '/who-we-are/' ] ] );
        ...
        $this->start_controls_section(
			'what_section',
			[
				'label' => esc_html__( 'What We Do', 'custom-hello-theme' ),
				'tab' => Controls_Manager::TAB_CONTENT,
			]
		);

        $this->add_control( 'what_title', [ 'type' => Controls_Manager::TEXTAREA, 'label' => 'Title', 'default' => 'We provide energy solutions with integrity and efficiency through our different business divisions.' ] );
        $this->add_control( 'what_read_more', [ 'type' => Controls_Manager::WYSIWYG, 'label' => 'Description', 'default' => '<p class="fs-s1">Montfort\'s interlinked divisions complement each other, providing integrated services that leverage their combined expertise. This synergy enhances our operational efficiency, enabling us to drive collective success in the global market and deliver exceptional value to our stakeholders.</p>' ] );

        $div_repeater = new Repeater();
        $div_repeater->add_control( 'idx', [ 'type' => Controls_Manager::TEXT, 'label' => 'Index' ] );
        $div_repeater->add_control( 'title', [ 'type' => Controls_Manager::TEXT, 'label' => 'Title' ] );
        $div_repeater->add_control( 'heading', [ 'type' => Controls_Manager::TEXTAREA, 'label' => 'Heading' ] );
        $div_repeater->add_control( 'link', [ 'type' => Controls_Manager::URL, 'label' => 'Link' ] );
        $div_repeater->add_control( 'row', [ 'type' => Controls_Manager::TEXT, 'label' => 'Grid Row' ] );
        $div_repeater->add_control( 'classes', [ 'type' => Controls_Manager::TEXT, 'label' => 'Col Classes' ] );

        $this->add_control( 'divisions', [
            'label' => 'Divisions',
            'type' => Controls_Manager::REPEATER,
            'fields' => $div_repeater->get_controls(),
            'default' => [
                [ 'idx' => '1', 'title' => 'Montfort Trading', 'heading' => 'Operating Efficiently by Leading with Innovation.', 'link' => [ 'url' => '/trading/' ], 'row' => '2', 'classes' => 'tb:col-start-1 tb:col-end-4 dk:col-start-5 dk:col-end-18 lg:col-start-7 lg:col-end-14' ],
                [ 'idx' => '2', 'title' => 'Montfort Capital', 'heading' => 'Identify and seize opportunities that maximise Value', 'link' => [ 'url' => '/capital/' ], 'row' => '3', 'classes' => 'tb:col-start-2 tb:col-end-5 dk:col-start-12 dk:col-end-24 lg:col-start-16 lg:col-end-23' ],
                [ 'idx' => '3', 'title' => 'Montfort Maritime', 'heading' => 'Powering Progress, Delivering Energy.', 'link' => [ 'url' => '/maritime/' ], 'row' => '4', 'classes' => 'tb:col-start-1 tb:col-end-4 dk:col-start-5 dk:col-end-18 lg:col-start-7 lg:col-end-14' ],
                [ 'idx' => '4', 'title' => 'Fort Energy', 'heading' => 'Advancing Innovation in Energy Investments', 'link' => [ 'url' => '/fort-energy/' ], 'row' => '5', 'classes' => 'tb:col-start-2 tb:col-end-5 dk:col-start-12 dk:col-end-24 lg:col-start-16 lg:col-end-23' ],
            ],
            'title_field' => '{{{ title }}}',
        ] );
        ...
        $this->start_controls_section(
			'global_section',
			[
				'label' => esc_html__( 'Global Connectivity', 'custom-hello-theme' ),
				'tab' => Controls_Manager::TAB_CONTENT,
			]
		);

        $this->add_control( 'global_title', [ 'type' => Controls_Manager::TEXTAREA, 'label' => 'Title', 'default' => 'Established in the world’s major trade hubs and financial markets with over 15 global offices, we connect and serve both emerging and mature markets worldwide.' ] );

        $point_repeater = new Repeater();
        $point_repeater->add_control( 'name', [ 'type' => Controls_Manager::TEXT, 'label' => 'Name' ] );
        $point_repeater->add_control( 'lng', [ 'type' => Controls_Manager::TEXT, 'label' => 'Longitude' ] );
        $point_repeater->add_control( 'lat', [ 'type' => Controls_Manager::TEXT, 'label' => 'Latitude' ] );

        $this->add_control( 'points', [
            'label' => 'Map Points',
            'type' => Controls_Manager::REPEATER,
            'fields' => $point_repeater->get_controls(),
            'default' => [
				[ 'name' => 'Switzerland', 'lng' => '4', 'lat' => '46.818188' ],
				[ 'name' => 'Istanbul', 'lng' => '24', 'lat' => '41.00824' ],
				[ 'name' => 'United Arab Emirates', 'lng' => '54.35495', 'lat' => '24.48818' ],
				[ 'name' => 'Nairobi', 'lng' => '36.828842', 'lat' => '-1.3026148' ],
				[ 'name' => 'Dar es Salam', 'lng' => '39.2803583', 'lat' => '-6.8160837' ],
				[ 'name' => 'Cape Town', 'lng' => '18.4172197', 'lat' => '-33.9288301' ],
				[ 'name' => 'Mumbai', 'lng' => '72.8281049', 'lat' => '18.9733536' ],
				[ 'name' => 'Karachi', 'lng' => '67.0207055', 'lat' => '24.8546842' ],
				[ 'name' => 'Maputo', 'lng' => '32.56745', 'lat' => '-25.966213' ],
				[ 'name' => 'Luxembourg', 'lng' => '6.1296751', 'lat' => '49.8158683' ],
				[ 'name' => 'Xiamen', 'lng' => '118.0853479', 'lat' => '24.4801069' ],
				[ 'name' => 'Singapore', 'lng' => '97', 'lat' => '1.352083' ],
			],
            'title_field' => '{{{ name }}}',
        ] );
            ...
            <div class="topChapters" data-chapter="TopChapters" data-chapter-first="true">
                ...
                <section class="hero" data-chapter="Hero" data-chapter-first="true" data-theme="dark" data-animation="Hero" data-astro-cid-226k2nfd>
					<div class="hero-inner" data-astro-cid-226k2nfd>
                        ...
                        <svg class="logo logo-dk" data-astro-cid-226k2nfd="true" width="667" height="80" viewBox="0 0 667 80" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <path d="M475.68 20.52H451.99V37.8H471.71V40.91H451.99V61.81H448.53V17.42H475.68V20.52Z" fill="#2D628C" style="fill:#2D628C;fill:color(display-p3 0.1765 0.3843 0.5490);fill-opacity:1;"/>
                            <path d="M403.92 20.52H420.18V17.41H384.2V20.52H400.46V61.63H403.92V20.52Z" fill="#2D628C" style="fill:#2D628C;fill:color(display-p3 0.1765 0.3843 0.5490);fill-opacity:1;"/>
                            <path d="M355.83 62.4799V17.3799H352.37V54.52L322.97 16.7H321.59V61.7799H325.05V24.83L354.45 62.4799H355.83Z" fill="#2D628C" style="fill:#2D628C;fill:color(display-p3 0.1765 0.3843 0.5490);fill-opacity:1;"/>
                            ...
                            <path d="M215.37 61.81V16.72H214.14L195.63 43.15L177.12 16.72H175.74V61.8H179.2V25.72L194.94 48H196.32L211.89 25.72V61.81H215.37Z" fill="#2D628C" style="fill:#2D628C;fill:color(display-p3 0.1765 0.3843 0.5490);fill-opacity:1;"/>
                            <path d="M129.43 39.75L128.77 5.04004L128.11 39.75L128.77 74.46L129.43 39.75Z" fill="#2D628C" style="fill:#2D628C;fill:color(display-p3 0.1765 0.3843 0.5490);fill-opacity:1;"/>
                            <path d="M67.72 24.15C69.87 24.15 71.49 22.41 71.49 20.39C71.49 18.24 69.74 16.63 67.72 16.63C66.24 16.63 64.89 17.43 64.35 18.78V18.91C64.22 19.04 64.22 19.18 64.22 19.31C61.93 24.95 54.93 25.22 54.93 25.22C53.18 25.22 51.7 26.42 51.16 27.91C49.81 32.34 46.04 32.47 44.29 32.2C43.89 32.07 43.34 32.07 42.94 32.07H42.4C42.13 32.07 41.86 32.2 41.6 32.2C39.98 32.47 36.22 32.33 34.73 28.04V27.91C34.19 26.3 32.71 25.22 30.96 25.22C30.96 25.22 24.09 24.82 21.81 19.31C21.81 19.04 21.68 18.91 21.54 18.64C20.87 17.44 19.66 16.49 18.17 16.49C16.15 16.49 14.4 18.23 14.4 20.25C14.4 22.27 16.15 24.01 18.17 24.01C18.17 24.01 25.3 24.41 27.05 29.79C27.32 31.53 28.67 33.01 30.55 33.15H31.09C31.36 33.15 31.63 33.15 31.89 33.02C33.64 33.15 36.6 33.96 37.54 38.25C37.54 38.52 37.67 38.79 37.67 39.05C38.07 40.25 38.75 41.34 39.82 42.14C39.82 42.14 43.99 45.09 40.09 50.32C40.09 50.45 39.96 50.45 39.96 50.59V50.72
                        </svg>
                        <div class="scroll-to-cta" data-astro-cid-226k2nfd>
                            <div class="scroll-to-cta-inner" data-astro-cid-226k2nfd>
                                <div class="scroll-to-cta-content-mb" data-astro-cid-226k2nfd>
                                    ...
                                    <span class="fs-cta-s" data-astro-cid-226k2nfd data-elementor-setting-key="scroll_text_mb"><?php echo esc_html( $settings['scroll_text_mb'] ); ?></span>
                                </div>
                                <div class="scroll-to-cta-content-dk" data-astro-cid-226k2nfd>
                                    <span class="fs-cta-s" data-astro-cid-226k2nfd data-elementor-setting-key="scroll_text_dk"><?php echo esc_html( $settings['scroll_text_dk'] ); ?></span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <section class="astro-va4abrey" data-chapter="WhoWeAre" id="WhoWeAre" data-label="Who we are" data-astro-cid-va4abrey>
					<div class="grid" data-astro-cid-va4abrey>
						<h2 data-animation="Title" class="fs-h2 uppercase tb:col-end-4 dk:col-start-10 dk:col-end-23 lg:col-start-14" data-astro-cid-va4abrey data-elementor-setting-key="who_title">
							<?php echo wp_kses_post( $settings['who_title'] ); ?>
						</h2>
						<div class="text-block dk:col-start-5 dk:col-end-13 lg:col-start-7" data-astro-cid-tbw6esjt>
							<div class="content fs-s1 montfort-navy-blue" data-astro-cid-tbw6esjt>
								<div class="content fs-s1 montfort-navy-blue" data-animation="TextBlock" data-astro-cid-tbw6esjt data-elementor-setting-key="who_content">
									<?php echo wp_kses_post( $settings['who_content'] ); ?>
								</div>
							</div>
							<a href="<?php echo esc_url( $settings['who_link_url']['url'] ); ?>" class="link-block montfort-navy-blue  undefined" data-animation="FadeIn" data-astro-cid-chamlvsj="true">
								...
								<span class="link-block-label" data-astro-cid-chamlvsj data-elementor-setting-key="who_link_label"><?php echo esc_html( $settings['who_link_label'] ); ?></span>
                ...
                <section class="astro-zdjhxtfb" data-chapter="WhatWeDo" id="WhatWeDo" data-label="What we do" data-astro-cid-zdjhxtfb>
					<div class="grid" data-astro-cid-zdjhxtfb>
						<h2 data-animation="Title" class="fs-h2 uppercase tb:col-end-4 dk:col-start-5 dk:col-end-19 lg:col-start-7 lg:col-end-17" data-astro-cid-zdjhxtfb data-elementor-setting-key="what_title">
							<?php echo wp_kses_post( $settings['what_title'] ); ?>
						</h2>
							...
							<div class="content-wrapper" data-astro-cid-mer3b7za style="--line-count: 4;">
								<div data-astro-cid-mer3b7za style="--line-count: 4;">
									<div class="content" data-astro-cid-mer3b7za style="--line-count: 4;">
										<div class="inner clamp fs-body montfort-navy-blue" data-astro-cid-mer3b7za style="--line-count: 4;" data-elementor-setting-key="what_read_more">
											<?php echo wp_kses_post( $settings['what_read_more'] ); ?>
										</div>
									</div>
					...
					<div class="grid divisions-container" data-astro-cid-zdjhxtfb>
						<?php foreach ( $settings['divisions'] as $item ) : ?>
							<div class="division-link-ship <?php echo esc_attr( $item['classes'] ); ?>" style="grid-row: <?php echo esc_attr( $item['row'] ); ?>;" data-astro-cid-r7rfymor>
								<div class="division-index white" data-animation="FadeIn" data-astro-cid-r7rfymor>
									<span class="fs-label" data-astro-cid-r7rfymor><?php echo esc_html( $item['idx'] ); ?></span>
								</div>
								<div class="division-content" data-astro-cid-r7rfymor>
									<h3 class="fs-h4 white" data-animation="FadeIn" data-astro-cid-r7rfymor><?php echo esc_html( $item['title'] ); ?></h3>
									<h4 class="fs-h3 white" data-animation="Title" data-animation-color="#ffffff" data-astro-cid-r7rfymor><?php echo esc_html( $item['heading'] ); ?></h4>
									<a href="<?php echo esc_url( $item['link']['url'] ); ?>" class="link-block white  undefined" data-animation="FadeIn" data-astro-cid-chamlvsj="true">
										<div class="arrow-wrapper left" data-astro-cid-chamlvsj>
											<div class="arrow-container left" data-astro-cid-chamlvsj>
												<svg data-astro-cid-chamlvsj="true" xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="none" viewBox="0 0 10 10" focusable="false" aria-hidden="true"><path fill="currentColor" d="M1 4.4a.6.6 0 0 0 0 1.2zm8.424 1.024a.6.6 0 0 0 0-.848L5.606.757a.6.6 0 1 0-.849.849L8.151 5 4.757 8.394a.6.6 0 1 0 .849.849zM1 5.6h8V4.4H1z"/></svg>
											</div>
										</div>
										<span class="link-block-label" data-astro-cid-chamlvsj><?php echo esc_html( $item['title'] ); ?></span>
										<div class="arrow-wrapper right" data-astro-cid-chamlvsj>
											<div class="arrow-container right" data-astro-cid-chamlvsj>
												<svg data-astro-cid-chamlvsj="true" xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="none" viewBox="0 0 10 10" focusable="false" aria-hidden="true"><path fill="currentColor" d="M1 4.4a.6.6 0 0 0 0 1.2zm8.424 1.024a.6.6 0 0 0 0-.848L5.606.757a.6.6 0 1 0-.849.849L8.151 5 4.757 8.394a.6.6 0 1 0 .849.849zM1 5.6h8V4.4H1z"/></svg>
											</div>
                ...
                <section class="astro-5vnrzpll" data-chapter="GlobalConnectivity" id="GlobalConnectivity" data-label="Global connectivity" data-theme-chapters="dark" data-cursor="draggable" data-cursor-down="dragging" data-animation="GlobalConnectivity" data-astro-cid-5vnrzpll>
					<div class="grid" data-astro-cid-5vnrzpll>
						<h2 class="fs-h2 uppercase montfort-navy-blue-2 tb:col-end-4 dk:col-start-7 dk:col-end-22 lg:col-start-10" data-label="Global connectivity" data-astro-cid-5vnrzpll data-elementor-setting-key="global_title">
							<?php echo wp_kses_post( $settings['global_title'] ); ?>
						</h2>
					</div>
					<?php foreach ( $settings['points'] as $point ) : ?>
						<p class="fs-cta-s uppercase" data-point data-longitude="<?php echo esc_attr( $point['lng'] ); ?>" data-latitude="<?php echo esc_attr( $point['lat'] ); ?>" data-astro-cid-u35nqrgz>
